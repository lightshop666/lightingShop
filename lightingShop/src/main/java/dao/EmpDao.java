...
public class EmpDao {
	...
	public int selectProductCnt() throws Exception {
		...
		// PreparedStatement
				...
				if(productCntRs.next()){
					totalRow = productCntRs.getInt(1); //행이 하나일때 index 1사용
				}
	...
	//3)회원 하나 상세 매소드
		public Customer selectCustomerOne(String id) throws Exception {
			...
			Connection conn = dbUtil.getConnection(); 
			// PreparedStatement
			String customerOneSql = "SELECT id, cstm_name, cstm_address,cstm_email, cstm_birth,cstm_phone,cstm_gender, cstm_rank,cstm_point,cstm_last_login,cstm_agree, createdate,updatedate from customert where id= ?";
