...
<!------------------------ 주문상품 -->
	...
	%>
	        ...
	        <p>상품 가격 : <%=decimalFormat.format( (int) (product.getProductPrice() * Integer.parseInt(productCnt[i])) )  %> 원</p><!-- 상품 원래 가격 * 상품 개수 --> 
	        <p>
	        <%
...
<!---------------- 결제
	...
	<div>
		<p>사용 가능 : <%=decimalFormat.format(totalPoint) %> P</p>
		<p>
...
<!---------------- 최종 결제 금액 -->
	<div>
		<h4>결제 금액</h4>
		<p>상품 금액 : <%= decimalFormat.format(totalPrice )%> 원</p>
		<p>배송비 : <%=decimalFormat.format(deliPrice) %> 원</p>
...
<!-------- 결제 버튼  넘겨줄 것 : productNo배열, productCnt배열, 최종금액 -->
	...
	%>
		...
		<button type="submit" id="orderButton">
			<span id="paymentAmount"><%=decimalFormat.format(totalPrice) %>원</span>결제
		</button>
	</form>
...
//포인트 적용시 최종 금액 변동
	...
	function calculateAmount() {
	  ...
	  // 최종 결제 금액을 화면에 표시
	  finalPriceElement.innerHTML = finalPrice.toLocaleString() + '원';
